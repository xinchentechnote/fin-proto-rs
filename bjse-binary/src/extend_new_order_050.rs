// Code generated by fin-protoc. DO NOT EDIT.
use binary_codec::*;
use bytes::{Buf, BufMut, Bytes, BytesMut};

#[derive(Debug, Clone, PartialEq)]
pub struct ExtendNewOrder050 {
    pub expiration_days: u16,
    pub expiration_type: u8,
    pub share_property: String,
}

impl BinaryCodec for ExtendNewOrder050 {
    fn encode(&self, buf: &mut BytesMut) {
        buf.put_u16_le(self.expiration_days);
        buf.put_u8(self.expiration_type);
        put_char_array(buf, &self.share_property, 2);
    }

    fn decode(buf: &mut Bytes) -> Option<ExtendNewOrder050> {
        let expiration_days = buf.get_u16_le();
        let expiration_type = buf.get_u8();
        let share_property = get_char_array(buf, 2)?;
        Some(Self {
            expiration_days,
            expiration_type,
            share_property,
        })
    }
}

#[cfg(test)]
mod extend_new_order_050_tests {
    use super::*;
    use bytes::BytesMut;

    #[test]
    fn test_extend_new_order_050_codec() {
        let original = ExtendNewOrder050 {
            expiration_days: 1234,
            expiration_type: 42,
            share_property: vec!['a'; 2].into_iter().collect::<String>(),
        };

        let mut buf = BytesMut::new();
        original.encode(&mut buf);
        let mut bytes = buf.freeze();

        let decoded = ExtendNewOrder050::decode(&mut bytes).unwrap();
        assert_eq!(original, decoded);
    }
}
