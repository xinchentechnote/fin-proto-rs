...
impl BinaryCodec for BusinessReject {
    fn encode(&self, buf: &mut BytesMut) {
        ...
        buf.put_i64_le(self.ref_seq_num);
        buf.put_u32_le(self.ref_msg_type);
        put_char_array(buf, &self.business_reject_ref_id, 10);
        buf.put_u16_le(self.business_reject_reason);
    ...
    fn decode(buf: &mut Bytes) -> Option<BusinessReject> {
        ...
        let ref_seq_num = buf.get_i64_le();
        let ref_msg_type = buf.get_u32_le();
        let business_reject_ref_id = get_char_array(buf, 10)?;
        let business_reject_reason = buf.get_u16_le();
